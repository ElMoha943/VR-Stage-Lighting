...
#if LUTBEAM_CALLBACK_STATE
    float4 callbackState : TEXCOORD55;
#endif
};
...
#if !LUTBEAM_EXTERNAL_CAMERA_DEPTH
...
#if LUTBEAM_CALLBACK_STATE
    beam.callbackState = callbackState;
#endif
...
float3 LUTBeamFrag(BeamData beam, float beamFalloff)
{
...
#if LUTBEAM_CALLBACK_UV_TRANSFORM
    entryNormalized.xy = LUTBeamCallbackUVTransform(entryNormalized.xy
    #if LUTBEAM_CALLBACK_STATE
        , beam.callbackState
    #endif
    );
    exitNormalized.xy = LUTBeamCallbackUVTransform(exitNormalized.xy
    #if LUTBEAM_CALLBACK_STATE
        , beam.callbackState
    #endif
    );
    ...
    float projectionFac = volFacNotHot;
#if LUTBEAM_VIEW_INDEPENDENT_PROJECTION
    // Surface projection attenuation must be based on the surface hit, not
    // the closest point on this camera ray through the volume.
    float3 surfaceFromSource = exitPos - float3(frustumNearZ, 0, 0);
    float surfaceDistance = length(surfaceFromSource) + frustumNearZ;
    float surfaceT = saturate(inverselerp(
        frustumNearZ - 0.06,
        frustumFarZ,
        surfaceDistance
    ));
    projectionFac = (1 - surfaceT) * (1 - surfaceT) * pow(surfaceT + 0.01, -1);
#endif
    ...
    if(hit && any(beam.colorGobo))
    {
        ...
        if(any(goboResult))
        {
            goboResult *= projectionFac * beam.colorGobo;

#if LUTBEAM_VIEW_INDEPENDENT_PROJECTION
            // Add the projected light directly. Sampling the screen grab here
            // makes brightness depend on view-dependent surface shading.
            col += goboResult;
#else
